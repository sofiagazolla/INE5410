...
void ponte_inicializar() {
    ...
    // Inicializando semáforos, sentido inicial = continente ----> ilha
    sem_init(&semaforo_ponte, 0, 1); // value como 1 para exlusão mútua (funcionar como um mutex)
...
void ponte_sair(veiculo_t *v) {
    ...
    // Quando o contador for igual a zero, significa que todos os veículos já saíram da ponte
    if (contador == 0) {
