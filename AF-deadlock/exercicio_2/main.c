...
void transferir(conta_t *origem, conta_t *destino, double valor) {
    ...
    // estabelece que a conta de menor id vai ser bloqueada antes
    if (origem->id > destino->id) { // se o id da origem for maior, ela é bloqueada antes
        pthread_mutex_lock(&origem->mutex);
        pthread_mutex_lock(&destino->mutex);
    } else { // se não, o destino é bloqueado antes
        pthread_mutex_lock(&destino->mutex);
        pthread_mutex_lock(&origem->mutex);
    }
