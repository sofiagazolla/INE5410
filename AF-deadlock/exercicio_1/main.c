...
void *worker1_func(void *arg) {
    for (int i = 0; i < WORKER_LOOPS; ++i) {
        ...
        // Worker 1 aguarda a liberação do semáforo para acessar a lista
        sem_wait(&sem_lista);
        ...
        // Libera o semáforo para que outra thread possa acessar a lista
        sem_post(&sem_lista);
...
void *worker2_func(void *arg) {
    for (int i = 0; i < WORKER_LOOPS; ++i) {
        ...
        // Worker 2 aguarda a liberação do semáforo para acessar a lista
        sem_wait(&sem_lista);
        ...
        // Libera o semáforo para que a outra thread possa acessar a lista
        sem_post(&sem_lista);
